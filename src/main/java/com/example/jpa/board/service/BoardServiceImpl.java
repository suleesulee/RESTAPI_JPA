...
import com.example.jpa.user.repository.MailTemplateRepository;
...
public class BoardServiceImpl implements BoardService{
    ...
    @Override
    public BoardType getByName(String boardName) {
        return null;
    }
    ...
    public ServiceResult addBoard(BoardTypeInput boardTypeInput) {
        ...
        if (boardType != null && boardTypeInput.getName().equals(boardType.getBoardName())){
            ...
            return ServiceResult.fail("동일한 게시판이 존재합니다.");
        }
    ...
    public ServiceResult setBoardTop(Long id, Boolean topYn) {
        ...
        if(board.isTopYn() == topYn) {
            if (topYn) {
                return ServiceResult.fail("이미 게시글이 최상단에 배치되어있습니다.");
            } else {
                return ServiceResult.fail("이미 게시글이 최상단 배치가 해제되어있습니다.");
            }
        }
    ...
    public ServiceResult setBoardHits(Long id, String email) {
        ...
        if(boardHitsRepository.countByBoardAndUser(board, user) > 0){
            return ServiceResult.fail("이미 조회했습니다.");
        }
    ...
    public ServiceResult setBoardLike(Long id, String email) {
        ...
        if(boardLikeCount > 0){
            return ServiceResult.fail("이미 좋아요했습니다.");
        }
    ...
    public ServiceResult scrap(Long id, String email) {
        ...
        BoardScrap boardScrap = BoardScrap.builder()
                .user(user)
                .boardId(board.getId())
                .boardType(board.getBoardType().getId())
                .boardTitle(board.getTitle())
                .boardContents(board.getContents())
                .boardRegDate(board.getRegDate())
                .regDate(LocalDateTime.now())
    ...
    public ServiceResult removeScrap(Long id, String email) {
        ...
        if(user.getId() != boardScrap.getUser().getId()){
            return ServiceResult.fail("이상 접근 Error");
        }
    ...
    public ServiceResult add(String email, BoardInput boardInput) {
        ...
        optionalMailTemplate.ifPresent((e)->{
            ...
            String contents = e.getTitle().replaceAll("\\{USER_NAME\\}", user.getUserName());
            //생략
    ...
    public ServiceResult replyBoard(Long id, BoardReplyInput boardReplyInput) {
        ...
        board.setReplyContents(boardReplyInput.getReplyContent());
        boardRepository.save(board);
        ...
        optionalMailTemplate.ifPresent((e)->{
            ...
            String contents = e.getTitle().replaceAll("\\{BOARD_TITLE\\}", board.getTitle());
            //생략
